...
AjaxSolr.GroupFacet = AjaxSolr.AbstractFacetWidget.extend({
  ...
  afterRequest: function () {
    ...
    // Update the classgroup on the search form dropdown menu
    var searchForm = this.manager.widgets['search'].target;
    $('input[value='+active+']', searchForm).attr('checked', 'true');
  ...
  clickHandler: function (facet) {
    ...
    return function (i) {
      ...
      var selectedGroup = facet.name;
      
      if (typeof selectedGroup != 'undefined') {
      	if (selectedGroup == 'Researchers' || selectedGroup == 'Groups' ) {
      ...
      self.manager.store.removeByKey('fq', self.field);
