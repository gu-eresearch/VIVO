...
AjaxSolr.ParameterStore = AjaxSolr.Class.extend(
  ...
  add: function (name, param) {
    ...
    if (this.params[name] === undefined) { //if it is the first paramter with this name
		this.params[name] = [ ];
	} 
  ...
  findByKey: function (name, key) {
    if (this.params[name] !== undefined) {
      if (this.isMultiple(name)) {
        var indices = [];
        for (var i = 0, l = this.params[name].length; i < l; i++) {
          if (AjaxSolr.equals(this.params[name][i].key, key)) {
            indices.push(i);
          }
        }
        return indices.length ? indices : false;
        ...
        if (AjaxSolr.equals(this.params[name].key, key)) {
          return name;
        }
      }
    }
  ...
  removeByKey: function (name, key) {
    var indices = this.findByKey(name, key);
    if (indices) {
      if (AjaxSolr.isArray(indices)) {
        for (var i = indices.length - 1; i >= 0; i--) {
          this.remove(name, indices[i]);
