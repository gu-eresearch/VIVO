...
<%
		...
		String  queryString = 
							"SELECT DISTINCT ?uri (STR(?Preferred) as ?preferred_name) (STR(?Given) as ?given_name) (STR(?Family) as ?family_name) (STR(?Email) as ?email) \n"+
							"WHERE\n"+
							"{\n"+
							"	{\n"+
							"		?uri a guhubext:ResearchHubPerson . \n"+
							"		?uri foaf:familyName ?Family . \n"+
							"		?uri foaf:givenName ?Given . \n"+
							"		?uri guhubext:preferredName ?Preferred . \n"+
							"		?uri core:email ?Email . \n"+
		...
		while(results.hasNext()){
		 	...
		 	RDFNode preferred_namenode = soln.get("preferred_name");
		 	RDFNode given_namenode = soln.get("given_name");
		 	RDFNode familynamenode = soln.get("family_name");
 			...
 			if (preferred_namenode != null) {
 				jsonObj.put("preferred_name", preferred_namenode.toString());		            	
 			}
 			if (given_namenode != null) {
 				jsonObj.put("given_name", given_namenode.toString());		            	
 			}
 			if (familynamenode != null) {
 				jsonObj.put("family_name", familynamenode.toString());		            	
 			}
 			if (emailnode != null) {
