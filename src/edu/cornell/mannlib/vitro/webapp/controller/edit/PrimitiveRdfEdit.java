...
public class PrimitiveRdfEdit extends VitroAjaxController {
    ...
    protected void doRequest(VitroRequest vreq,
        ...
        try {
            ...
            Model writeModel = null;
            if( ! StringUtils.isEmpty( modelName )){ //use specified model
                writeModel = getWriteModel(vreq, modelName);
        	} else { //use standard model
        	    writeModel = getWriteModel(vreq);
        	}
        	
        	Model depResRetractions = DependentResourceDeleteJena.getDependentResourceDeleteForChange(toBeAdded,toBeRetracted, writeModel);
            toBeRetracted.add(depResRetractions);
            processChanges(editorUri, writeModel, toBeAdded, toBeRetracted);
    ...
    private Model getWriteModel(VitroRequest vreq, String modelName){
        ModelSelector modelSelector = new IdModelSelector( modelName );
    	return modelSelector.getModel(vreq,getServletContext());  
    }
