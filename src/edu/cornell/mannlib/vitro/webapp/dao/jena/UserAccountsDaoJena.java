...
public class UserAccountsDaoJena extends JenaBaseDao implements UserAccountsDao {
	...
	public UserAccount getUserAccountByUri(String uri) {
		...
		try {
			...
			// add additional permissions form ldap
			if (u.isExternalAuthOnly()) {
			    // but only check ldap if external auth only
	                    setPermissionsFromLdap(u);
			}		
			return u;
		} finally {
			getOntModel().leaveCriticalSection();
	...
	private void setPermissionsFromLdap(UserAccount u) {
	    if (u == null || u.getExternalAuthId() == null) {
	        return;
	    }
	    Set<String> permissions = u.getPermissionSetUris();
	    try {
	        ...
	        DirContext dc = (DirContext) ((LdapSource)envCtx.lookup("ldap/SignonTree")).getLdapContext();
	        // TODO make some of these hardcoded constants configurable
	        Attributes attrs = dc.getAttributes("cn=" + u.getExternalAuthId() + ",ou=active,ou=People,O=Griffith University");
	        Attribute groups = attrs.get("groupMembership");

	        String admin_group = "cn=LML - RH Admin,ou=Groups,o=Griffith University";
	        String editor_group = "cn=LML - RH Editors,ou=Groups,o=Griffith University";
	            
	        if (groups.contains(admin_group)) {
	            permissions.add(PermissionSetsLoader.URI_DBA);
	        } else if (groups.contains(editor_group)) {
	            permissions.add(PermissionSetsLoader.URI_EDITOR);
	        }
	        dc.close();
	        // we had a successful login.... so give user minimal permissions anyway even if ldap fails.
	        // TODO: should do some sort of notification in this case
	    } catch (Exception e) {
	        log.warn("There was an LDAP problem. Fallback to original behaviour: " + e.getMessage());
	        permissions.add(PermissionSetsLoader.URI_SELF_EDITOR);
	    }
	    u.setPermissionSetUris(permissions);
    }
