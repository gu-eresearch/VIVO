...
public class SelfEditingConfiguration {
	...
	public static String externalAuthIdToURI(String namespace, String externalAuthId) {
          // convert snumber to EID
          if (externalAuthId == null) {
              return null;
          }
          if (!externalAuthId.startsWith("s")) {
              return null;
          }
          Integer id = null;
          try {
              // remove leading 's' and convert to int
              id = new Integer(externalAuthId.substring(1));
          } catch (NumberFormatException e)  {
              log.info("can't parse unique id number '" + externalAuthId + "'");
              // it's not a number return it as is
              return null;
          }
          // reformat as 7 digit number
          NumberFormat formatter = new DecimalFormat("0000000");
          // convert eid into hash
          String hash = DigestUtils.md5Hex("EIF_GriffithUni" + formatter.format(id));
          String uri = namespace + "n" + hash;
          return uri;
      }
