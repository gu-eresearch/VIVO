 ...
 *
 */
public abstract class AddRoleToPersonTwoStageGeneratorGU extends BaseEditConfigurationGenerator implements EditConfigurationGenerator {
	
	private Log log = LogFactory.getLog(AddRoleToPersonTwoStageGeneratorGU.class);
...
//    	        getN3ForActivityType(),                              
    ...
    private String getN3RoleDescAssertion() {
    	return "?role <http://purl.org/dc/terms/description> ?roleDesc .";
    }
    ...
    private void setSparqlQueries(EditConfigurationVTwo editConfiguration, VitroRequest vreq) throws Exception {        
        ...
        map.put("roleDesc", getRoleDescQuery(vreq));
	...
	private String getRoleDescQuery(VitroRequest vreq) {
		String query = "SELECT ?existingRoleDesc WHERE { \n" +
				"?role  <http://purl.org/dc/terms/description> ?existingRoleDesc . }";
		return query;
	...
	private void setFields(EditConfigurationVTwo editConfiguration, VitroRequest vreq, String predicateUri) throws Exception {
    	...
    	getRoleDescField(editConfiguration, vreq, fields);
...
//    	field.setLiteralOptions(fieldLiteralOptions);
	...
	private void getRoleDescField(EditConfigurationVTwo editConfiguration,
			VitroRequest vreq, Map<String, FieldVTwo> fields) {
		String fieldName = "roleDesc";
   ...
   private void addPreprocessors(EditConfigurationVTwo editConfiguration, WebappDaoFactory wadf) {
	...
	private final String DateTimeValueURI =   VIVO_NS + "dateTime";	
	 ...
	 private static String ALL_EXISTING_ROLE_LABELS_QUERY = ""
        + "    PREFIX guhubext: <http://griffith.edu.au/ontology/hubextensions/> \n"
        + "    PREFIX core: <http://vivoweb.org/ontology/core#> \n" 
        + "    PREFIX rdfs:  <http://www.w3.org/2000/01/rdf-schema#> \n" 
        + "    SELECT  DISTINCT (str(?Label) as ?label)  WHERE { \n" 
        + "     { ?role a guhubext:AcademicExperienceRole . ?role rdfs:label ?Label . } \n"
        + "     UNION { ?role a guhubext:ProfessionalExperienceRole . ?role rdfs:label ?Label . } \n"
        + "     UNION { ?role a guhubext:InternalServiceRole . ?role rdfs:label ?Label . } \n"
        + "     UNION { ?role a guhubext:ExternalServiceRole . ?role rdfs:label ?Label . } \n"
        + "     UNION { ?role a guhubext:VisitingAppointmentRole . ?role rdfs:label ?Label . } \n"
        + "     UNION { ?role a guhubext:EditorialRole . ?role rdfs:label ?Label . } \n"
        + "     UNION { ?role a guhubext:ExternalAdvisoryRole . ?role rdfs:label ?Label . } \n"
        + "     UNION { ?role a guhubext:GovernmentRepresentationRole . ?role rdfs:label ?Label . } \n"
        + "     #UNION { ?role a core:EducationalTraining . ?role rdfs:label ?Label . } \n"
        + "    } \n" 
        + "    ORDER BY ?Label ";
        
    private static String ALL_EXISTING_ORG_LABELS_QUERY = ""
        + "    PREFIX guhubext: <http://griffith.edu.au/ontology/hubextensions/> \n"
        + "    PREFIX core: <http://vivoweb.org/ontology/core#>  \n"
        + "    PREFIX rdfs:  <http://www.w3.org/2000/01/rdf-schema#> \n" 
        + "    SELECT  DISTINCT (str(?Label) as ?label)  WHERE {  \n"
        + "     { ?role a guhubext:AcademicExperienceRole . ?role core:roleIn ?org . ?org rdfs:label ?Label . } \n"
        + "     UNION { ?role a guhubext:ProfessionalExperienceRole . ?role core:roleIn ?org . ?org rdfs:label ?Label . } \n"
        + "     UNION { ?role a guhubext:InternalServiceRole . ?role core:roleIn ?org . ?org rdfs:label ?Label . } \n"
        + "     UNION { ?role a guhubext:ExternalServiceRole . ?role core:roleIn ?org . ?org rdfs:label ?Label . } \n"
        + "     UNION { ?role a guhubext:VisitingAppointmentRole . ?role core:roleIn ?org . ?org rdfs:label ?Label . } \n"
        + "     UNION { ?role a guhubext:EditorialRole . ?role core:roleIn ?org . ?org rdfs:label ?Label . } \n"
        + "     UNION { ?role a guhubext:ExternalAdvisoryRole . ?role core:roleIn ?org . ?org rdfs:label ?Label . } \n"
        + "     UNION { ?role a guhubext:GovernmentRepresentationRole . ?role core:roleIn ?org . ?org rdfs:label ?Label . } \n"
        + "     #UNION { ?role a core:EducationalTraining . ?role core:trainingAtOrganization ?org . ?org rdfs:label ?Label . } \n"
        + "    }  \n"
        + "    ORDER BY ?Label ";    
        
    
    private List<Map<String, String>> getAutoCompleteLabelsForRole(VitroRequest vreq) {
          
        String queryStr = ALL_EXISTING_ROLE_LABELS_QUERY;
        log.debug("All Role label Query string is: " + queryStr);
        ...
        try {
            ...
            while (results.hasNext()) {
                ...
                if (node != null) {
                    labels.add(QueryUtils.querySolutionToStringValueMap(soln));        
                }
            }
        } catch (Exception e) {
            log.error(e, e);
    ...
    private List<Map<String, String>> getAutoCompleteLabelsForOrgs(VitroRequest vreq) {
          
        String queryStr = ALL_EXISTING_ORG_LABELS_QUERY;
        log.debug("All Org label Query string is: " + queryStr);
        List<Map<String, String>> labels = new ArrayList<Map<String, String>>();
        try {
            ResultSet results = QueryUtils.getQueryResults(queryStr, vreq);
            while (results.hasNext()) {
	...
	public void addFormSpecificData(EditConfigurationVTwo editConfiguration, VitroRequest vreq) {
		...
		//Add data for autocompletes
        List<Map<String,String>> allExistingRoleLabels = getAutoCompleteLabelsForRole(vreq);
        formSpecificData.put("allExistingRoleLabels",allExistingRoleLabels);
        
        List<Map<String,String>> allExistingOrgLabels = getAutoCompleteLabelsForOrgs(vreq);
        formSpecificData.put("allExistingOrgLabels",allExistingOrgLabels);
