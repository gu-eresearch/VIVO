...
public class PostEditCleanupController extends FreemarkerHttpServlet{
    ...
    protected static ResponseValues doPostEditRedirect( VitroRequest vreq , String entityToReturnTo){
        ...
        if( entityToReturnTo != null ){
            ...
            path += getPredicateGroupAnchor( vreq, editConfig );
     ...
     * Adds a attribute to the request to indicate which group the predicate which was edited belongs to.
     * This attribute is used by some controllers to send the browser to the
     * tab on the page relevant to the predicate.
     * 
     * Never returns null, it will return an empty string if there is nothing. 

     */

    public static String getPredicateGroupAnchor( VitroRequest vreq, EditConfigurationVTwo config){

        if( vreq == null || config == null )

            return "";  

        
        //Get propgroup local name if it exists
        String predicateGroupName = getPredicatePropertyGroup(vreq, config);

        if( predicateGroupName != null && predicateGroupName.length() > 0){

            String predicateAnchor = "#" + predicateGroupName;

            vreq.setAttribute("predicateAnchor", predicateAnchor);
			
			log.debug(predicateAnchor);
            return predicateAnchor;
    ...
    //Get predicate prop group anchor
	public static String getPredicatePropertyGroup(VitroRequest vreq, EditConfigurationVTwo editConfig) {
		String predicateGroupName = null;
		if( editConfig != null ){
			...
			if( predicateUri != null ){
				try{
					...
					ObjectPropertyDao opDao = vreq.getFullWebappDaoFactory().getObjectPropertyDao();
					PropertyGroupDao pgDao = vreq.getFullWebappDaoFactory().getPropertyGroupDao();
					PropertyGroup pGroup = null;
					
					ObjectProperty op = opDao.getObjectPropertyByURI(predicateUri);
					if (op != null) {
						pGroup = pgDao.getGroupByURI(op.getGroupURI());							
					} else {
						DataProperty dp = dpDao.getDataPropertyByURI(predicateUri);
						if (dp != null) {
							pGroup = pgDao.getGroupByURI(dp.getGroupURI());		
						}
					}

					if (pGroup != null) {			
						predicateGroupName = pGroup.getName();
					}
